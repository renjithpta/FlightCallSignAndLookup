...
utils.prepareErrorResponse = (error, code, message) => {
 ...
 utils.disconnectGateway= async function(){
...
utils.getChannelNetwork = async (orgName) => {

    console.log(">>>connectGatewayFromConfig:  ".yellow.big,orgName);
    ...
    try {
       ...
       let filePathLocal= path.resolve(
        ...
        console.log(("walletpath = " + walletpath).yellow);
...
utils.connectGatewayFromConfig = async (orgName) => {

    console.log(">>>connectGatewayFromConfig:  ",orgName);
...
utils.submitPrivateTx = async(contract, txName, rate , params) => {
    console.log(">>>utils.submitTx..."+txName+" ("+params+")");
   
    let result =  null;
   if( rate && rate != null ){

   result  = contract.createTransaction(txName).setTransient(rate).submit(params.toString());

   }else{

    if(params.length == 1)
    result  = contract.submitTransaction(txName,params.toString());
    
    else
    result  = contract.submitTransaction(txName.toString(),params[0].toString() , params[1].toString());
...
utils.setPrivateUserContext = async (userid, pwd) => {
    console.log('\n>>>setUserContext...');
    ...
    try {
        ...
        network = await userGateway.getNetwork(configdata["channel_name"]);
        contract = await network.getContract(configdata["smart_contract_name"],"KycPrivateAssetContract");
